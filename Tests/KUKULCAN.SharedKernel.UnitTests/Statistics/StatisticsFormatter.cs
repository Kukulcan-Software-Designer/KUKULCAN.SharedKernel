...
public static class StatisticsFormatter
{
    ...
    public static string FormatPercentage(
        double value,
        int decimalDigits = StatisticsConstants.DefaultDecimalDigits)
    {
    ...
    /// Formats a ratio.
    /// </summary>
    public static string FormatRatio(
        double value,
        int decimalDigits = StatisticsConstants.DefaultDecimalDigits)
    {
        return value.ToString(
        ...
        TimeSpan duration)
    {
        ...
        return $"{duration.TotalDays:N2} d";
    }

    /// <summary>
    /// Formats a distribution entry.
    /// </summary>
    public static string FormatDistribution<TKey>(
        DistributionEntry<TKey> entry)
        where TKey : notnull
    {
        ...
        where TKey : notnull
    {
        ...
        return string.Join(
            separator,
            entries.Select(FormatDistribution));
    }
        ...
        StatisticsSnapshot snapshot)
    {
        ...
        return string.Join(
            Environment.NewLine,
            snapshot.Values.Select(static x => $"{x.Key} = {x.Value}"));
    }

    /// <summary>
    /// Formats a nullable value.
    /// </summary>
    public static string FormatNullable<T>(
        T? value,
        ...
        int decimalDigits = StatisticsConstants.DefaultDecimalDigits)
    {
        return value.HasValue
            ? FormatNumber(value.Value, decimalDigits)
            : "<null>";
    }
